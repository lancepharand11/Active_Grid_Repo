...
for train_fraction_idx, train_fraction in enumerate(np.linspace(min_train_fraction,max_train_fraction,n_steps)):
    ...
    best_scaler_x = None
    best_scaler_y = None
    rel_rmse_turb_int = np.zeros(n_models)
    rel_rmse_L_ux = np.zeros(n_models)
    ...
    for fold, (train_idx, val_idx) in enumerate(SS.split(X_all)):
        ...
        with torch.no_grad():
            ...
            rel_rmse_turb_int[fold] = np.sqrt(np.sum(rel_err_turb_int**2) / rel_err_turb_int.size)
            rel_rmse_L_ux[fold] = np.sqrt(np.sum(rel_err_L_ux**2) / rel_err_L_ux.size)
        
            print(f"Fold {fold + 1} Relative RMSEs:")
            print(f"    Turbulence Intensity: {rel_rmse_turb_int[fold]:.4f}")
            print(f"    L_ux / M: {rel_rmse_L_ux[fold]:.4f}")
            ...
            #     best_rel_rmse_turb_int = rel_rmse_turb_int
            #     best_rel_rmse_L_ux = rel_rmse_L_ux
    ...
    #             f"{best_rel_rmse_turb_int:.4f}\t"
    #             f"{best_rel_rmse_L_ux:.4f}\n"
    #             )
    
    # Print training data size
    train_data_size[train_fraction_idx] = X_all.size(0)*train_fraction
    ...
    overall_fraction_acceptable[train_fraction_idx] = np.mean(fraction_acceptable)
    print(f"Overall fraction of acceptable predictions: {overall_fraction_acceptable[train_fraction_idx]}")
